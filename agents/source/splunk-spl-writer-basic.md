...
- Correct: `earliest="12/11/2025:03:15:00"`
- Wrong: `earliest="2025-12-11T03:15:00Z"`
...
Let's assume that Time Zero is 12/5/2025:01:00:00 and we decided to use a 15 minutes window for both before and after (total of 30 minutes in the final time range).
Execute this `tstats` command:

```spl
| tstats count where earliest="12/5/2025:00:45:00" latest="12/5/2025:01:15:00" by sourcetype, index
```
...
```spl
(index=main sourcetype=WinEventLog) OR (index=windows sourcetype=XmlWinEventLog) earliest="12/5/2025:00:45:00" latest="12/5/2025:01:15:00"
| stats count
```
...
```spl
index=main sourcetype IN (WinEventLog, XmlWinEventLog) earliest="12/5/2025:00:45:00" latest="12/5/2025:01:15:00" 178.31.28.111 ("login" OR "failure" OR "denied") | head 1
```
...
```spl
index=main sourcetype IN (WinEventLog, XmlWinEventLog) earliest="12/5/2025:00:45:00" latest="12/5/2025:01:15:00" 178.31.28.111 ("login" OR "failure" OR "denied")
| stats count
```
