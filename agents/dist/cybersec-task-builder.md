...
OCSF alerts use ISO 8601 timestamps (e.g., `2026-04-26T03:30:42Z`), but Splunk `earliest` and `latest` parameters require `MM/DD/YYYY:HH:MM:SS` format. Use the `format_timestamp` native function:
...
2. **Ad-Hoc Execution Test** (MANDATORY):
   - **Step 2a: Create data_samples** (FOLLOW THESE STEPS EXACTLY):
     ...
     4. **Add ALL accessed fields to data_samples**:
        - If script uses `get_primary_observable_value(input)`, add `"observables": [{"type_id": 2, "type": "IP Address", "value": "91.234.56.101"}]`
     ...
     6. **Example structure**:
        ```json
        {
          "data_samples": [
            {
              "triggering_event_time": "2026-04-26T03:30:42Z",
              "finding_info": {"title": "Suspicious IP Communication from 91.234.56.101", "uid": "sample-001", "analytic": {"name": "Suspicious IP Communication", "type": "Rule", "type_id": 1}},
              "observables": [{"type_id": 2, "type": "IP Address", "value": "91.234.56.101"}],
              "evidences": [{"src_endpoint": {"ip": "91.234.56.101"}}],
              "enrichments": {}
            }
          ]
        }
