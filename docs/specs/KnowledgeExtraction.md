...
These rules govern the **extract_and_transform** (Node 4a) and **merge_with_existing** (Node 4b) prompts. They are derived from proven patterns observed when transforming analyst investigations into runbooks, generalized for arbitrary source material.
...
|----------|--------|----------|
| Specific IPs, domains, hashes | **Strip** — replace with symbolic placeholders | `192.168.1.50` → `<source_ip>`, `attacker.example` → `<suspicious_domain>` |
