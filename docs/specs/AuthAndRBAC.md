...
**MCP client configuration:** Claude Code and other MCP clients pass the Bearer token in the `Authorization` header, which is supported by the MCP Streamable HTTP transport spec.
