...
class TestCompleteSPLFormat:
    ...
    def test_complete_spl_format_example(self):
        ...
        ioc_filter = generator._build_entity_ioc_filter(
            "user@company.com",
            ["192.168.1.100", "http://evil.com/malware", "bad-domain.com"],
        )

        expected_filter = '("user@company.com") AND ("192.168.1.100" OR "http://evil.com/malware" OR "bad-domain.com")'
        ...
        # - IOCs are quoted
        assert '"user@company.com"' in complete_spl
        assert '"192.168.1.100"' in complete_spl
        assert '"http://evil.com/malware"' in complete_spl
    ...
    def test_real_world_spl_examples(self):
        ...
        auth_entity_filter = generator._build_entity_ioc_filter(
            "john.doe@company.com", ["192.168.1.50", "10.0.0.100", "vpn.company.com"]
        )

        assert (
            auth_entity_filter
            == '("john.doe@company.com") AND ("192.168.1.50" OR "10.0.0.100" OR "vpn.company.com")'
        )
    ...
    def test_spl_format_documentation(self):
        ...
        search (index=main AND sourcetype=pan:threat) OR (index=firewall AND sourcetype=cisco:asa)
               ...
               ("user@company.com") AND ("192.168.1.100" OR "http://evil.com/path" OR "malicious.domain.com")
        """.strip()

        # This is what we expect the SPL to look like
        assert "index=main AND sourcetype=pan:threat" in expected_format
        assert '"user@company.com"' in expected_format
        assert '"http://evil.com/path"' in expected_format
