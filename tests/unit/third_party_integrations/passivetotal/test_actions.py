...
class TestHelpers:
    ...
    def test_validate_date_range_valid(self):
        assert _validate_date_range("2024-01-01", "2024-02-01") is None

    def test_validate_date_range_reversed(self):
        result = _validate_date_range("2024-03-01", "2024-01-01")
        assert result is not None
        assert "after" in result

    def test_validate_date_range_bad_from(self):
        result = _validate_date_range("bad-date", "2024-01-01")
        assert result is not None
...
class TestLookupDomainAction:
    ...
    async def test_success(self, action):
        ...
        passive_resp = _mock_response({"firstSeen": "2024-01-01", "results": []})
    ...
    async def test_reversed_date_range(self, action):
        result = await action.execute(
            domain="example.com", **{"from": "2024-06-01", "to": "2024-01-01"}
        )
        assert result["status"] == "error"
        assert result["error_type"] == "ValidationError"
...
class TestLookupIpAction:
    ...
    async def test_success(self, action):
        ...
        ssl_resp = _mock_response(
            {"results": [{"sha1": "abc123", "firstSeen": "2024-01-01"}]}
        )
...
class TestGetHostComponentsAction:
    ...
    async def test_with_date_range(self, action):
        ...
        result = await action.execute(
            query="example.com", **{"from": "2024-01-01", "to": "2024-06-01"}
        )
