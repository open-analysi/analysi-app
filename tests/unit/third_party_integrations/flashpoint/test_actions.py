    ...
    first_resp = _mock_response(
        {
            "results": [
                ...
                {"type": "domain", "value": "attacker.example"},
            ],
            "scroll_id": None,
        }
    )
...
async def test_search_indicators_url_quoting(search_indicators):
    ...
    await search_indicators.execute(
        attribute_type="URL",
        attribute_value="http://attacker.example/payload",
        limit=5,
    )

    call_kwargs = search_indicators.http_request.call_args.kwargs
    assert 'url=="http://attacker.example/payload"' in call_kwargs["params"]["search_fields"]
