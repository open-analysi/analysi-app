...
class TestDetectIndicatorType:
    ...
    def test_detects_url(self):
        ...
        assert _detect_indicator_type("https://attacker.example") == ENDPOINT_URL
...
class TestUrlReputationAction:
    ...
    async def test_url_reputation_success(self, action):
        ...
        mock_response.json.return_value = {
            "Zone": "Red",
            "UrlGeneralInfo": {
                "Url": "http://attacker.example/phishing",
                "Categories": ["Phishing"],
                "HasApt": False,
                "RelatedAptReports": [],
            },
        ...
        result = await action.execute(url="http://attacker.example/phishing")

        assert result["status"] == "success"
        assert result["data"]["url"] == "http://attacker.example/phishing"
        assert result["data"]["zone"] == "Red"
