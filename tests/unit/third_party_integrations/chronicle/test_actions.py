...
async def test_list_ioc_details_domain_success(mock_credentials, mock_settings):
    ...
    with (
        ...
        result = await action.execute(
            artifact_indicator="Domain Name", value="attacker.example"
        )

    assert result["status"] == "success"
    assert result["artifact_indicator"] == "Domain Name"
    assert result["value"] == "attacker.example"
...
async def test_list_assets_invalid_time_range(mock_credentials, mock_settings):
    ...
    result = await action.execute(
        ...
        end_time="2026-04-26T00:00:00Z",  # End before start
    )
...
async def test_list_assets_missing_required_params(mock_credentials, mock_settings):
    ...
    result = await action.execute(artifact_indicator="Domain Name", value="attacker.example")
...
async def test_list_events_with_reference_time(mock_credentials, mock_settings):
    ...
    with (
        ...
        result = await action.execute(
            ...
            start_time="2026-04-26T00:00:00Z",
            end_time="2026-04-27T00:00:00Z",
            reference_time="2026-04-26T12:00:00Z",
        )
...
async def test_list_iocs_success(mock_credentials, mock_settings):
    ...
    mock_response = {
        "response": {
            "matches": [
                {"artifact": {"domainName": "attacker.example"}},
    ...
    with (
        ...
        result = await action.execute(start_time="2026-04-26T00:00:00Z")

    assert result["status"] == "success"
    assert result["total_iocs"] == 2
    assert result["start_time"] == "2026-04-26T00:00:00Z"
...
async def test_domain_reputation_malicious(mock_credentials, mock_settings):
    ...
    assert result["status"] == "success"
    assert result["domain"] == "attacker.example"
