...
class TestBlockSenderAction:
    ...
    async def test_success(self, action):
        resp = {
            "data": [
                {"sender": "bad@attacker.example", "to": "user@corp.example", "type": "block"}
            ],
            "fail": [],
        }
        ...
        with patch(OAUTH_MODULE, new_callable=AsyncMock, return_value="token"):
            result = await action.execute(sender="bad@attacker.example", to="user@corp.example")

        assert result["status"] == "success"
        assert result["data"]["sender"] == "bad@attacker.example"
    ...
    async def test_sends_block_action(self, action):
        ...
        with patch(OAUTH_MODULE, new_callable=AsyncMock, return_value="token"):
            await action.execute(sender="bad@attacker.example", to="user@corp.example")
    ...
    async def test_missing_sender(self, action):
        result = await action.execute(to="user@corp.example")
    ...
    async def test_missing_to(self, action):
        result = await action.execute(sender="bad@attacker.example")
...
class TestUnblockSenderAction:
    ...
    async def test_success(self, action):
        ...
        with patch(OAUTH_MODULE, new_callable=AsyncMock, return_value="token"):
            result = await action.execute(
                sender="sender@example.com", to="user@corp.example"
            )
    ...
    async def test_sends_permit_action(self, action):
        ...
        with patch(OAUTH_MODULE, new_callable=AsyncMock, return_value="token"):
            await action.execute(sender="sender@example.com", to="user@corp.example")
...
class TestGetManagedUrlAction:
    ...
    async def test_found(self, action):
        url_list = {
            "data": [
                {"id": "url-abc", "domain": "attacker.example", "action": "block"},
        ...
        assert result["data"]["domain"] == "attacker.example"
...
class TestAddManagedUrlAction:
    ...
    async def test_success_block(self, action):
        ...
        with patch(OAUTH_MODULE, new_callable=AsyncMock, return_value="token"):
            result = await action.execute(url="https://attacker.example/phishing")
    ...
    async def test_default_values_in_payload(self, action):
        ...
        with patch(OAUTH_MODULE, new_callable=AsyncMock, return_value="token"):
            await action.execute(url="https://attacker.example")
    ...
    async def test_missing_credentials(self):
        ...
        result = await action.execute(url="https://attacker.example")
