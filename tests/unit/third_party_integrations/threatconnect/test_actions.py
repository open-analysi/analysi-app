...
class TestLookupUrlAction:
    ...
    async def test_success(self, action):
        api_response = {
            "status": "Success",
            "data": [{"id": 222, "text": "http://attacker.example", "type": "URL"}],
            "count": 1,
        }
        mock_resp = _json_response(api_response)
        with patch(
...
class TestLookupEmailAction:
    ...
    async def test_success(self, action):
        api_response = {
            "status": "Success",
            "data": [{"id": 333, "address": "bad@attacker.example", "type": "EmailAddress"}],
            "count": 1,
        }
        mock_resp = _json_response(api_response)
        with patch(
            ...
            result = await action.execute(email="bad@attacker.example")
...
class TestCreateIndicatorAction:
    ...
    async def test_create_domain_indicator(self, action):
        api_response = {
            "status": "Success",
            "data": {"id": 555, "hostName": "attacker.example", "type": "Host"},
            "message": "Created",
        }
        mock_resp = _json_response(api_response)
        with patch.object(
            ...
            result = await action.execute(
                indicator_value="attacker.example", indicator_type="domain"
            )

        assert result["status"] == "success"
        assert mock_req.call_args.kwargs["json"]["hostName"] == "attacker.example"
    ...
    async def test_create_url_indicator(self, action):
        api_response = {
            "status": "Success",
            "data": {"id": 777, "text": "http://attacker.example"},
        }
        mock_resp = _json_response(api_response)
        with patch.object(
            action, "http_request", new_callable=AsyncMock, return_value=mock_resp
            ...
            result = await action.execute(
                indicator_value="http://attacker.example", indicator_type="url"
            )

        assert result["status"] == "success"
        assert mock_req.call_args.kwargs["json"]["text"] == "http://attacker.example"

    @pytest.mark.asyncio
    async def test_create_email_indicator(self, action):
        api_response = {
            "status": "Success",
            "data": {"id": 888, "address": "bad@attacker.example"},
        }
        mock_resp = _json_response(api_response)
        with patch.object(
            action, "http_request", new_callable=AsyncMock, return_value=mock_resp
            ...
            result = await action.execute(
                indicator_value="bad@attacker.example", indicator_type="email"
            )

        assert result["status"] == "success"
        assert mock_req.call_args.kwargs["json"]["address"] == "bad@attacker.example"
