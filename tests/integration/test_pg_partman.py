...
Note on partition naming: Some tables were renamed in V110 (e.g.,
...
async def _get_child_partitions(session: AsyncSession, parent_table: str) -> list[str]:
    ...
    renamed (e.g., alert_analysis -> alert_analyses in V110).
    """
...
class TestPgPartmanSetup:
    ...
    async def test_default_partitions_exist(
        ...
        Uses pg_inherits to find defaults because renamed tables (V110)
        may have child partitions whose names don't start with the
        ...
        fresh migration run because V094's create_parent() adopted
        ...
        for table in ALL_PARTITIONED_TABLES:
            ...
            if len(defaults) == 0:
                # Default missing (known V094 edge case for tables that had
