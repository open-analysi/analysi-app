"""
Integration tests for reconciliation fixes.
...
async def _create_paused_alert(
    ...
    analysis = AlertAnalysis(
        ...
        alert_id=alert.id,
        tenant_id=tenant_id,
        status="paused",  # DB constraint changed paused_workflow_building -> paused
...
async def test_mark_stuck_alert_failed_is_no_op_for_paused_workflow_building(db):
    ...
    assert analysis.status == "paused"  # paused_workflow_building -> paused
...
async def test_mark_failed_alone_does_not_persist_without_commit(db):
    ...
    assert fresh_analysis is not None
    assert fresh_analysis.status == "paused"  # paused_workflow_building -> paused
...
async def test_update_alert_status_does_not_affect_other_alerts(db):
    ...
    assert analysis2.status == "paused"  # paused_workflow_building -> paused
