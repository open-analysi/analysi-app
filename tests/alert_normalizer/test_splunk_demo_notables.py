...
class TestPowershellExploit:
    ...
    def test_network_info(self, normalizer, notable):
        ...
        assert result.network_info.src_ip == "91.234.56.6"
        assert result.network_info.dest_ip == "10.10.30.8"  # Exchange Server 2
    ...
    def test_iocs_extracted(self, normalizer, notable):
        ...
        assert "91.234.56.6" in ioc_values
...
class TestSqlInjection:
    ...
    def test_network_info(self, normalizer, notable):
        ...
        result = normalizer.to_alertcreate(notable)
        assert result.network_info is not None
        assert result.network_info.src_ip == "91.234.56.17"
    ...
    def test_iocs_include_attacker_ip(self, normalizer, notable):
        ...
        assert result.iocs is not None
        ioc_values = [ioc.value for ioc in result.iocs]
        assert "91.234.56.17" in ioc_values
...
class TestXssAttack:
    ...
    def test_network_info(self, normalizer, notable):
        ...
        result = normalizer.to_alertcreate(notable)
        assert result.network_info is not None
        assert result.network_info.src_ip == "91.234.56.42"
    ...
    def test_iocs_extracted(self, normalizer, notable):
        ...
        assert result.iocs is not None
        ioc_values = [ioc.value for ioc in result.iocs]
        assert "91.234.56.42" in ioc_values
...
class TestLsCommand:
    ...
    def test_iocs_exclude_internal_ip(self, normalizer, notable):
        ...
        assert "10.10.20.46" not in ioc_values
        # Should have the external IP and URL
        assert "203.198.7.15" in ioc_values
        has_url_ioc = any("corpsite.example" in v for v in ioc_values)
        assert has_url_ioc
...
class TestWhoamiCommand:
    ...
    def test_network_info(self, normalizer, notable):
        ...
        result = normalizer.to_alertcreate(notable)
        assert result.network_info is not None
        assert result.network_info.src_ip == "91.234.56.87"
    ...
    def test_iocs_extracted(self, normalizer, notable):
        ...
        assert result.iocs is not None
        ioc_values = [ioc.value for ioc in result.iocs]
        assert "91.234.56.87" in ioc_values
...
class TestIdorAttack:
    ...
    def test_network_info(self, normalizer, notable):
        ...
        result = normalizer.to_alertcreate(notable)
        assert result.network_info is not None
        assert result.network_info.src_ip == "91.234.56.118"
    ...
    def test_iocs_extracted(self, normalizer, notable):
        ...
        assert result.iocs is not None
        ioc_values = [ioc.value for ioc in result.iocs]
        assert "91.234.56.118" in ioc_values
